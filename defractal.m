%%
%Defractal method Salem et al. (2014)
%%
%By 
...
%%%%%%%%%%%%%%%%%%%% Description %%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%% INVERSION 
     % 1- Import data [GEOtiff (*.tif) or radially averaged power spectrum (*.txt)] and flight height (km)
     % example : defractal('synt_200.tif',0) or defractal('RAPS.txt',0)
...
%%%
     ...
     % View plots : Corrected radially averaged power spectrum and corrected scaled spectrum 
...
%%
% fractal exponent 
b2=input('Enter the alpha exponent: ');
clc;
%%
...
f2=figure;
plot(w2,ps2,'k.-');
ylabel('ln($\frac{A(k)}{{2\pi}k^{2}}$)','Interpreter', ...
...
for k = linspace(exp(M-4),exp(M+4),1000) 
    r=[r,k];
...
hold(ax3,'on')
if (fileid(data) == 1 || cfint == 1)
    p4=fill([w2', fliplr(w2')],[pw2'+ci1',fliplr(pw2'-ci1')],...
        [0.6 0.6 0.6],'EdgeColor','k','Facealpha',0.5, 'LineStyle','none');
    fill([w2', fliplr(w2')],[a2(1:size(a2),2)'+ci1',...
        fliplr(a2(1:size(a2),2)'-ci1')],...
        [0.6 0.6 0.6],'Edgecolor','k','Facealpha',0.3); %, 'LineStyle','none');
end
p1=plot(w2,a2(1:size(a2),2),'g-','Linewidth',1,'Markersize',12);
...
xlabel('k ($\frac{1}{km}$)','Interpreter','latex', ...
    'FontSize',12,'FontName','Times New Roman');
if (fileid(data) == 1 || cfint == 1)
    legend([p1 p2 p3 p4],{'Observed spectrum','Corrected spectrum',...
        ['Synthetic spectrum (\alpha = ' num2str(b2) ')'],'95% confidence interval'})
elseif fileid(data) == 0
    legend([p1 p2 p3],{'Observed spectrum','Corrected spectrum',...
        ['Synthetic spectrum (\alpha = ' num2str(b2) ')']})
end
hold(ax3,'off')
%
subplot(2,2,4)
rectangle('Position',[0,0,1,1],'FaceColor','w')
str = {'Values',['\alpha = ' num2str(b2)],...
...
% function raps(img)
%
% Computes and plots radially a power
% spectrum of magnetic data in a GEOtif image
% of the amplitde spectrum
...
w=x(1:size(x),1);
ps=x(1:size(x),2)+log((w.^b1)./(w.^2));% for Z0
pw=x(1:size(x),2)+log(w.^b1);% for Zt
ci=x(1:size(x),3);
end 
%%
%%
